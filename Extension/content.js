...
function blurVideos() {
    chrome.storage.sync.get({ keywords: [] }, (result) => {
        ...
        // Get video elements from both grid layout and search results
        const gridVideos = document.querySelectorAll("ytd-rich-item-renderer.style-scope.ytd-rich-grid-renderer");
        const searchVideos = document.querySelectorAll("ytd-video-renderer.style-scope.ytd-item-section-renderer");
        
        // Combine both types of video elements
        const allVideos = [...gridVideos, ...searchVideos];

        allVideos.forEach((video, index) => {
            console.log("Video is processed", video);
            
            // Try multiple selectors to find the title element
            const titleSelectors = [
                '.yt-lockup-metadata-view-model__heading-reset a span',
                '#video-title',
                'a#video-title',
            ...
            if (!title) {
                console.log(`No title found for video ${index}`);
                return;
            }
            ...
            if (keywords.some(keyword => title.includes(keyword.toLowerCase()))) {
                console.log(`Blocking video with title: "${title}"`);
                // Apply a blur effect and make the video unclickable
                video.style.filter = 'blur(10px)';
                video.style.pointerEvents = 'none';
                video.style.opacity = '0.5';
                
                // Change title color if title element exists
                if (titleElement) {
